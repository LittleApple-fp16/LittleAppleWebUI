...
from ..infer.draw import draw_with_workdir, draw_with_workdir_kohya
...
def export_workdir(workdir: str, export_dir: str, n_repeats: int = 2,
                   ...
                   model_hash: Optional[str] = None, ds_repo: Optional[str] = None, is_kohya: Optional[bool] = False):
    if not is_kohya:
        ...
        with open(os.path.join(export_dir, 'README.md'), 'w', encoding='utf-8') as f:
            ...
            for step in steps[::-1]:
                ...
                for dname in d_names:
                    file = Path(os.path.join(str(step), 'previews', f'{dname}.png')).as_posix()
        ...
        logging.info(f'Starting export trained artifacts of {name!r}')
        model_hash = model_hash or KNOWN_MODEL_HASHES.get(pretrained_model, None)
        if model_hash:
            logging.info(f'Model hash {model_hash!r} detected for model {pretrained_model!r}.')
        dataset_info = None
